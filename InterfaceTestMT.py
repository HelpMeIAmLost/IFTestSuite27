...
class InterfaceTestMT(object):
    ...
    def update_internal_signals(self):
        ...
        if self.conn is not None:
            print('Updating the internal_signals table of the interface database..')
            sys.stdout.flush()
            ...
            for row in rows:
                ...
                with open('{}/application.map'.format(self.map_folder), 'r') as fp:
                    for line in fp:
    ...
    def send_once(self, bus, msg):
        ...
        while response_message is None and tries < 10:
            if msg.data[0] == 0xFF:
                ...
                print('Failed connecting to the XCP slave! Connect retry #{}'.format(tries + 1))
            elif msg.data[0] == 0xFE:
                logging.info("(InterfaceTestMT) XCP slave disconnect retry {}".format(tries))
                print('Failed disconnecting from the XCP slave! Disconnect retry #{}'.format(tries + 1))
            sys.stdout.flush()
            bus.send(msg)
            response_message = self.check_xcp_response(bus, slave_id)
            tries += 1
    ...
    def run(self):
        ...
        while not g_update_finished:
            ...
            if timeout_ms == (self.duration * 1000):
                ...
                print("{}  Output signal update failed!".format(round(g_input_timestamp_s + 1, 4)))
                sys.stdout.flush()
                    ...
                    if g_value_updated and \
                        ...
                        if actual_value == g_expected_value:
                            ...
                            if g_output_timeout_counter == 10:
                                ...
                                print("{}  Output signal update failed!".format(round(current_time, 4)))
                                sys.stdout.flush()
...
if not os.path.exists(args.map_folder):
    ...
    if not dbc_variant_folder_found:
        print('{} folder not found in the DBC folder!'.format(str(args.variant).upper()))
        sys.stdout.flush()
    elif not dbc_files_found:
        print('DBC files for {} not found in the DBC folder!'.format(str(args.variant).upper()))
        sys.stdout.flush()
    else:
        max_retry = 0
        ...
        if str(args.update_address).lower() == 'yes':
            if interface_test.update_internal_signals() == 0:
                print('Internal signals information were not updated! Aborting test..')
                sys.stdout.flush()
                sys.exit()
            if debug:
                sys.exit()
        ...
        print('*************************************************************************')
        print('  This version does not include testing of CAN, VP and DebugCAN signals  ')
        print('*************************************************************************')
        sys.stdout.flush()
                ...
                if module_name_o != io_pairing_row[3]:
                    if retry_count == 0:
                        if module_name_o != '' and log_to_output is not None:
                            log_to_output.close()
                            print('Finished tests for {}'.format(module_name_o))
                        print('Starting tests for {}'.format(io_pairing_row[3]))
                    else:
                        if module_name_o != '' and log_to_output is not None:
                            log_to_output.close()
                            print('Finished re-tests for {}'.format(module_name_o))
                        print('Re-testing failed test items for {}'.format(io_pairing_row[3]))
                    sys.stdout.flush()
                    first_entry = True
                ...
                if retry_count == 0:
                    ...
                    if skip_this_test_item:
                        print('{}: Skipped {} -> {} - {}'.format(
                            io_pairing_row[3], io_pairing_row[2], io_pairing_row[4], skip_reason))
                        sys.stdout.flush()
                    ...
                    if source['data_type'] == 'float32' and temp_data_type.find('int') != -1:
                        print(
                            '{}: Skipped {} -> {} - No tests yet for not matching data types ({} -> {})..'.format(
                                io_pairing_row[3], io_pairing_row[2],
                                io_pairing_row[4], source['data_type'], temp_data_type))
                        sys.stdout.flush()
                        execute_sql(db_connection,
                                    '''UPDATE io_pairing SET status=?, result=?, notes=? WHERE id=?;''',
                                    ('Skipped', 'N/A',
            ...
            if max_retry > 0:
                ...
                if retry_count > max_retry:
                    ...
                    io_pairing = execute_sql(db_connection,
                                             ...
                                             select=True)
                    print('The script is set to re-test failed test items {} time(s)'.format(max_retry))
                    print('Running retry {} of {}..'.format(retry_count, max_retry))
            else:
                IF_test_finished = True
            sys.stdout.flush()
        ...
        print('Test Results')
        print('-----------------------------------')
        print('{} of {} items tested'.format(tested_count, io_pairing_count))

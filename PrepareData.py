...
def create_data_list(input_file, variant, dbc_folder):
    ...
    if debug:
        print(u"Creating data frame from {}".format(input_file))
    else:
    ...
    cdl_data_frame.rename(columns={u'対象モデル': 'target_model_name'}, inplace=True)
    cdl_data_frame.rename(columns={'SigName(MDL)': 'model_signal_name'}, inplace=True)
    cdl_data_frame.rename(columns={u'入力元': 'source_modname'}, inplace=True)
    cdl_data_frame.rename(columns={'SigName(CAN/LIN/MDL)': 'source_signame'}, inplace=True)
    cdl_data_frame.rename(columns={u'型[配列サイズ]': 'raw_data_type'}, inplace=True)
    cdl_data_frame.rename(columns={u'出力先': 'destination_modname'}, inplace=True)
    ...
    cdl_data_frame['raw_data_type'] = replace(cdl_data_frame, 'raw_data_type', u'single[５]', 'single[5]')
    ...
    # Unnecessary text
    # HALTフラグ -> np.nan
    cdl_data_frame['source_signame'] = replace(cdl_data_frame, 'source_signame', u'HALTフラグ', np.nan)
    # FSモデルに出力追加の予定 -> np.nan
    cdl_data_frame['source_signame'] = replace(cdl_data_frame, 'source_signame', u'FSモデルに出力追加の予定', np.nan)
    # 車種パラに追加予定-> np.nan
    cdl_data_frame['source_signame'] = replace(cdl_data_frame, 'source_signame', u'車種パラに追加予定', np.nan)
    if debug:
        print u"Removed 'HALTフラグ', 'FSモデルに出力追加の予定' and '車種パラに追加予定' in source signal name"
    else:
    ...
    invalid3 = cdl_data_frame['raw_data_type'] == u'ー'
    invalid4 = cdl_data_frame['raw_data_type'] == u'―'
    ...
    invalid4 = cdl_data_frame['source_modname'] == u'ー'
    invalid5 = cdl_data_frame['source_modname'] == u'―'
    ...
    invalid4 = cdl_data_frame['source_signame'] == u'ー'
    invalid5 = cdl_data_frame['source_signame'] == u'―'
    ...
    invalid4 = cdl_data_frame['destination_modname'] == u'ー'
    invalid5 = cdl_data_frame['destination_modname'] == u'―'
    ...
    invalid4 = cdl_data_frame['destination_signame'] == u'ー'
    invalid5 = cdl_data_frame['destination_signame'] == u'―'
    ...
    if update_external_signals(variant, dbc_folder) == 0:
        ...
        print('External signals information were not updated! Aborting test..')
        sys.stdout.flush()
        sys.exit()
...
if debug:
    parser.add_argument('-i', dest='input_file', default=u'【SASB連-2018-21】外部IF定義書_20181228リリース.xlsx')

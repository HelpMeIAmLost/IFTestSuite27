...
def data_type_info(data_type):
    ...
    if data_type == np.float32:
        return [
            long(float_to_hex(np.finfo(np.float32).max), 16),
            long(float_to_hex(np.finfo(np.float32).min), 16),
            long(float_to_hex(1.23456789), 16)
                ]
    else:
...
def hex_to_float(h):
    ...
    return unpack('!f', bytearray.fromhex(h))[0]
...
def execute_sql(conn, sql_statement, values=None, select=False, count=False, just_one=False):
    ...
    try:
        ...
        if values is None:
            ...
            if np.nan in values or '-' in values or u'―' in values or u'ー' in values:
                pass
            else:
